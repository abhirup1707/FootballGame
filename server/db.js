...
function migratePostgres() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS friends (id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, friend_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, status TEXT NOT NULL DEFAULT 'pending', created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP, UNIQUE(user_id, friend_id));
    CREATE TABLE IF NOT EXISTS ad_rewards (id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, reward_key TEXT NOT NULL DEFAULT 'coins', claimed_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP);
    CREATE TABLE IF NOT EXISTS ad_watches (id SERIAL PRIMARY KEY, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, reward_key TEXT NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP);
    CREATE TABLE IF NOT EXISTS match_history (id SERIAL PRIMARY KEY, player_a_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, player_b_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, score_a INTEGER NOT NULL DEFAULT 0, score_b INTEGER NOT NULL DEFAULT 0, winner_id INTEGER REFERENCES users(id) ON DELETE SET NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP);
  ...
  try { db.prepare("CREATE TABLE IF NOT EXISTS match_history (id SERIAL PRIMARY KEY, player_a_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, player_b_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, score_a INTEGER NOT NULL DEFAULT 0, score_b INTEGER NOT NULL DEFAULT 0, winner_id INTEGER REFERENCES users(id) ON DELETE SET NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP)").run(); } catch {}
...
function upgrade() {
  ...
  if (version < 11) {
    db.exec("CREATE TABLE IF NOT EXISTS ad_watches (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, reward_key TEXT NOT NULL, created_at TEXT NOT NULL DEFAULT (datetime('now')))");
    db.exec("CREATE INDEX IF NOT EXISTS idx_ad_watches_user ON ad_watches(user_id)");
    const cols = db.prepare("PRAGMA table_info(ad_rewards)").all().map((c) => c.name);
    if (!cols.includes("reward_key")) db.exec("ALTER TABLE ad_rewards ADD COLUMN reward_key TEXT NOT NULL DEFAULT 'coins'");
  }
  if (version < 12) {
    db.exec("CREATE TABLE IF NOT EXISTS match_history (id INTEGER PRIMARY KEY AUTOINCREMENT, player_a_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, player_b_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, score_a INTEGER NOT NULL DEFAULT 0, score_b INTEGER NOT NULL DEFAULT 0, winner_id INTEGER REFERENCES users(id) ON DELETE SET NULL, created_at TEXT NOT NULL DEFAULT (datetime('now')))");
  }
  db.prepare("INSERT INTO meta (key, value) VALUES ('schema_version', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value").run(SCHEMA_VERSION);
}
upgrade();
