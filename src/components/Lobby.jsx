...
export default function Lobby({ setRoom, onBack }) {
  ...
  const { user } = useAuth();
  const [playerName] = useState(savedSession?.playerName || user?.username || "");
  ...
  const createRoom = () => { if (!playerName.trim()) return alert("Enter your manager name first"); sessionRef.current = { playerName:playerName.trim(), resumeToken:newToken() }; socket.emit("createRoom", { playerName:playerName.trim(), mode, matchMode, goalLimit, timeLimit, resumeToken:sessionRef.current.resumeToken }); };
