...
export default function Lobby({ setRoom }) {
  ...
  const joiningFromInvite = Boolean(initialCode);
  ...
  useEffect(() => {
    const created = (code) => { setRoomCode(code); saveSession(code, playerName.trim()); window.history.replaceState({}, "", `?room=${code}`); setStatus("Room created — share the match link with your opponent."); };
