...
function DirectionButtons({ onChoose }) { return <div className="direction-row">{directions.map((direction) => <button key={direction} onClick={() => onChoose(direction)}>{direction === "LEFT" ? "←" : direction === "RIGHT" ? "→" : "↑"}<span>{direction.toLowerCase()}</span></button>)}</div>; }
...
export default function PenaltyShootout({ room, data }) {
  ...
  const choose = (direction) => socket.emit("submitPenaltyDirection", { roomCode:room.roomCode, direction });
  ...
  const keeperDive = shootout.choices?.[defendingId] || result?.dive;
