...
import { fireGoalConfetti, fireWinConfetti } from "../confetti";
...
export default function Match({ room, initialData }) {
  const [data, setData] = useState(initialData), [locked, setLocked] = useState(false), [now, setNow] = useState(Date.now()), [flash, setFlash] = useState(null), [deadlineKey, setDeadlineKey] = useState(null), [replay, setReplay] = useState(null);
  const trailRef = useRef([]);
  useEffect(() => {
    const update = (next) => setData((old) => ({ ...old, ...next, receivedAt:Date.now() }));
    const goal = (next) => { fireGoalConfetti(); setFlash({ type:"goal", ...next }); setReplay({ scorer: next.scorer, shot: next.shot, minute: Math.floor((data?.elapsedMs || 0) / 60000) }); setTimeout(() => setFlash(null), 2200); setTimeout(() => setReplay(null), 2800); };
  ...
  const deadlineLeft = (data.match.deadlineLeft || 0) - (now - (data.receivedAt || now));
  const remaining = Math.max(0, deadlineLeft / 1000);
  const choose = (move) => { setLocked(true); socket.emit("submitMove", { roomCode:room.roomCode, move }); };
  trailRef.current = [...trailRef.current.filter((p) => Date.now() - p.t < 600), { x: ballX, y: ballY, t: Date.now() }];
  ...
  return <main className="match-page">
    ...
    <section className="match-layout">
      <div className="pitch-view"><div className="match-stage"><span>{goalTime ? "FINAL SHOT" : `PASS ${data.match.passCount + 1} / 5`}</span><h1>{carrier?.name} has the ball</h1><div className="pass-track">{[1,2,3,4,5].map((step) => <i key={step} className={step <= data.match.passCount ? "done" : ""}>{step}</i>)}</div></div><div className="single-pitch"><div className="centre-circle" /><div className="penalty top" /><div className="penalty bottom" />{renderTeam(aId,"red")}{renderTeam(bId,"blue")}{trail.slice(0, -1).map((p, i) => <span key={p.t} className="ball-trail" style={{ left:`${p.x}%`, top:`${p.y}%`, opacity: (i + 1) / trail.length * 0.5, transform: `scale(${(i + 1) / trail.length * 0.6})` }} />)}<motion.div className="moving-ball" animate={{ left:`${ballX}%`, top:`${ballY}%` }} transition={{ type:"spring", stiffness:70, damping:10 }}>⚽</motion.div>{flash && <motion.div className={`${flash.type}-overlay`} initial={{ opacity:0,scale:.7 }} animate={{ opacity:1,scale:1 }}><span>{flash.type === "goal" ? "GOAL!" : flash.type === "save" ? "GREAT SAVE!" : "INTERCEPTED!"}</span><small>{flash.type === "goal" ? flash.scorer : flash.type === "save" ? `${flash.keeper} denies ${flash.shooter}` : `${flash.interceptor} steals it from ${flash.receiver}`}</small></motion.div>}</div></div>
      <aside className="match-control"><div className="carrier-card"><small>{goalTime ? "FINAL SHOT" : "CURRENT BALL CARRIER"}</small><b>{carrier?.name}</b><span>{carrier?.rating} OVR</span>{data.match.lastPass && <div className="pass-announcement" aria-label={`Latest pass for ${data.match.lastPass.team}`}><small>{data.match.lastPass.team}</small><b>{data.match.lastPass.passer} <i>→</i> {data.match.lastPass.receiver}</b></div>}</div><p className="instruction">{isAttacking ? (goalTime ? "Pick the corner and take the shot." : "Choose the next pass — or launch a long shot.") : (goalTime ? "Read the shot and cover a side." : "Predict the pass — or press the ball.")}</p>{!locked && <div className={`move-timer ${remaining < 1.5 ? "warning" : ""}`}><div className="move-timer-track"><div className="move-timer-bar" style={{ width:`${(remaining / 7) * 100}%` }} /></div><span>{remaining.toFixed(1)}s</span></div>}<div className={`move-options ${goalTime ? "direction-options" : "player-options"}`}>{options.map((option) => { const direction = typeof option === "string"; return <button key={direction ? option : option.id} disabled={locked} onClick={() => choose(direction ? option : option.id)}>{direction ? <><strong>{option === "LEFT" ? "←" : option === "RIGHT" ? "→" : "↑"}</strong><small>{option.toLowerCase()}</small></> : <><i>{option.rating}</i><span>{option.name}</span><small>{option.position}</small></>}</button>; })}{!goalTime && <button key="special" className="special" disabled={locked} onClick={() => choose(isAttacking ? "LONG_SHOT" : "PRESS")}>{isAttacking ? <><strong>⚡</strong><span>Long Shot</span><small>{carrier?.shooting ?? carrier?.rating} finishing</small></> : <><strong>⛔</strong><span>Press</span><small>smother the shot</small></>}</button>}</div>{locked && <p className="choice-wait">Locked in — waiting for the other player.</p>}<div className="match-feed"><strong>Match story</strong>{(data.commentary || []).map((line,index) => <p key={index}>{line}</p>)}</div></aside>
    </section>
    {replay && <motion.div className="goal-replay-card" initial={{ opacity: 0, scale: 0.6, rotateY: -20 }} animate={{ opacity: 1, scale: 1, rotateY: 0 }} exit={{ opacity: 0 }} transition={{ type: "spring", stiffness: 200, damping: 16 }}>
      <div className="goal-replay-badge">⚽ GOAL REPLAY</div>
      <div className="goal-replay-scorer">{replay.scorer}</div>
      <div className="goal-replay-detail">{replay.shot ? `${replay.shot} finish` : "Scored!"}{replay.minute ? ` · ${replay.minute}'` : ""}</div>
      <div className="goal-replay-shine" />
    </motion.div>}
  </main>;
}
